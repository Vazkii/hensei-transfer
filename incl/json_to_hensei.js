...
function __weapons(ctx, weapons) {
    ...
    for(k in weapons) {
        ...
        if(wpId.length > 0) {
            ...
            if('transcend' in obj)
                __put(ctx, 'grid_weapons', gwId, '', {weapon: {uncap_level: 6, transcendence_step: obj['transcend']}}, true);
            ...
            if('awakening' in obj) {
                ...
                if(awkObj)
                    __post(ctx, 'update_uncap', gwId, '', {weapon: {awakening_id: awkObj['id'], awakening_level: awkLvl}}, true);
            }
